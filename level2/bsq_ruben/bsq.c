...
static int  load_elem(FILE *f, t_elem *e)
{
    if (fscanf(f, "%d %c %c %c", &e->n, &e->emp, &e->obs, &e->ful) != 4 || e->n <= 0)
        return (-1);
    if (e->emp == e->obs || e->emp == e->ful || e->obs == e->ful)
        return (-1);
    return (0);
}
...
int execute_bsq(FILE *f)
{
    ...
    if (load_elem(f, &e) == -1) return (-1);
    if (load_map(f, &m, &e) == -1) return (-1);
    if (solve(&m, &e) == -1) { free_map(m.g); return (-1); }
    free_map(m.g);
    return (0);
}